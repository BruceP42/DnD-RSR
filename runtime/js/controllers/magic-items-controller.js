/* ---------------------------------------------------------
Path: runtime/js/controllers/magic-items-controller.js
File: magic-items-controller.js
Version: V2.3
Data Schema: V1.1
System: D&D Reference System – Reference System Runtime (RSR)
Module/Role: Page controller for magic-items reference view;
  manages URL-driven state, rendering via referenceService,
  and interaction binding for the magic-items domain
Dependencies:
...
Changelog:
  V2.3:
    - appMode=edit renderReference stub replaced with live navigation handler. Row click navigates to ?domain=magic-items&appMode=edit&id=<id>.
    - Added edit-record click handler in init() on #card-pane.
    - Edit button on the magic item card navigates to ?domain=magic-items&appMode=edit&id=<id>.
  V2.2:
  ...
  V2.0:
    - Added appMode to viewState (default "play"); serialized as
      appMode URL param (distinct from renderMode / mode param)
    - parseURL reads appMode param; defaults to "play"
    - buildURL serializes appMode param
    - renderFromState: in play mode with selectedId set, renders
      split-pane layout — table stays in #results, card renders
      into #card-stack; adds split-active class to #view-container
    - renderFromState: in all other cases clears #card-stack,
      removes split-active class, behaves as before
    - bindInteractions: in play mode, row click sets selectedId
      only — does not change viewType or renderMode
    - bindInteractions: in manage mode, row click behaves as
      before (transitions to entity/card view)
    - renderCardPane: new private function; renders entity card
      into #card-stack and wires close button
    - injectBackButton: now a no-op in play mode (close button
      in card pane replaces it)
  V1.2:
    - Imported renderFilterBar from runtime/js/_core/filter-utils.js
    - parseURL now collapses filter_<field>_min / filter_<field>_max
      pairs into range objects { min, max } in the filter map
    - parseURL now recognises filter_<field>=true/false as booleans
    ...
    - renderFromState calls renderFilterBar after table render;
      clears #filters div in entity view
  V1.1:
    - Imported bindTableSortClicks from runtime/js/_core/sort-utils.js
    - bindInteractions now calls bindTableSortClicks when renderMode
      is "table"
...
function parseURL() {
  ...
  return {
    ...
    domain:     params.get("domain")  || "magic-items",
          ...
          bindTableRowSelection({
            ...
            renderReference: ({ selectedId }) => {
              window.location.href =
                `?domain=magic-items&appMode=edit&id=${encodeURIComponent(selectedId)}`;
            }
          });
        }
    ...
    history.replaceState(null, "", buildURL(viewState));
        ...
        if (cardPane) {
          cardPane.addEventListener("click", e => {
            ...
            if (id) {
              window.location.href =
                `?domain=magic-items&appMode=edit&id=${encodeURIComponent(id)}`;
            }
          });
        }
